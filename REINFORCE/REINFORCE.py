...
class Policy(nn.Module):
    # This class is about policy network:
    # input is state, output is probability distribution of actions
...
class REINFORCE(object):
    ...
    def choose_action(self, s, deterministic):
        ...
        if deterministic:  # We use the deterministic policy during the evaluating
            ...
            a = np.random.choice(range(self.action_dim),
                                 p=prob_weights)  # Sample the action according to the probability distribution
            return a
    ...
    def learn(self, ):
        G = []  # G stores return value at certain time, G[t] = ut
        g = 0
        ...
        for t in range(len(self.episode_r)):
            ...
            policy_loss = -pow(self.GAMMA, t) * g * torch.log(a_prob[a])  # g(s,a|theta) = u_t * ln_pi(a_t|s_t; theta)
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(
        log_dir='runs/REINFORCE/REINFORCE_env_{}_number_{}_seed_{}'.format(env_name[env_index], number,
        ...
        while not done:
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                ...
                if evaluate_num % 10 == 0:
                    render_policy(env_evaluate, agent)
                    np.save(
