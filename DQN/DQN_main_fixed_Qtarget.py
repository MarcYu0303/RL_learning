...
class TrainManager():
    ...
    def __init__(self, env, episodes=900, lr=0.001, gamma=0.9, e_greed=0.1, bufferCap=1000, start_bufferCap=200):
        self.env = env
    ...
    def train_episode(self):
        ...
        while True:
            ...
            if done: break
        self.reward_memo.append(total_reward)
        return total_reward
    ...
    def plot_reward_diagram(self):
        plt.figure(1)
        plt.clf()
        plt.title('Training...')
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        if len(self.reward_memo) > 100:
            data = torch.from_numpy(np.array(self.reward_memo))
            means = data.unfold(0, 100, 1).mean(1).view(-1)
            means = torch.cat((torch.zeros(99), means))
            plt.plot(means)
        else:
            plt.plot(self.reward_memo)
        plt.show()
...
if __name__ == '__main__':
    ...
    tm.train()
    tm.plot_reward_diagram()
    # tm.evaluate_model('DQN_episode299_reward500.0.pth')